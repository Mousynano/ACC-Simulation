...
class Car{
    ...
    constructor(x, y, width, height, controlType, maxSpeed=maxVelocity, color=getRandomColor(), evaluator=Evaluator.iae){
        this.controller = new ClassicalACC();
        this.evaluator = evaluator;
        this.acc = new AdaptiveCruiseControl(this.controller, this.evaluator);
        ...
        if(!this.damaged){
            ...
            if(this.controlType != 'DUMMY'){
                ...
                if (allFalse){ // Kalau User ngga megang setir berarti ACC ambil alih
                    const u = this.acc.accUpdate(
                        this.vr, 
                        (leadVV == undefined) ? 0 : leadVV, 
                        maxVelocity, 
                        this.yr, 
                        (this.leadYR[1] == undefined) ? (this.yr - 1000) : this.leadYR[1], 
                        this.safeDistance
                    ); 

                    const v_ego, a_ego, Ft = this.physics(v_ego, u, dt)

                    // this.ar = this.acc.u; // Di sini letak dimana ACC memberi kendalinya
                    // this.av = this.ar / tScale;
                    // this.vr += this.av;
                    // this.obj_val = this.acc.obj_val;
                    // this.fitness = this.obj_val;
